...
extension RegistrySchemaV1 {
    ...
    final class Doctor: Employee, User {
        ...
        var basicService: PricelistItem.Short?
        var serviceDuration: TimeInterval = 600
        var defaultCabinet: Int = 1
        var salary: RegistrySchemaV1.Salary = RegistrySchemaV1.Salary.pieceRate(rate: 0.4)
        ...
        init(
            ...
            defaultCabinet: Int,
            salary: RegistrySchemaV1.Salary,
            agentFee: Double = 0,
        ...
        func charge(as role: KeyPath<RenderedService, AnyEmployee?>, amount: Double) {
            switch role {
            case \.performer:
                balance += amount
            case \.agent:
...
extension RegistrySchemaV2 {
    @Model
    final class Doctor: RegistrySchemaV3.Employee, User {
        let id: UUID = UUID()
        var secondName: String = ""
        var firstName: String = ""
        var patronymicName: String = ""
        ...
        var defaultCabinet: Int = 1
        var salary: RegistrySchemaV1.Salary = RegistrySchemaV1.Salary.pieceRate(rate: 0.4)
        var doctorSalary: RegistrySchemaV3.Salary = RegistrySchemaV3.Salary.pieceRate(rate: 0.4)
        private(set) var agentFee: Double = 0
        private(set) var agentFeePaymentDate: Date = Date.now
        var balance: Double = 0
        var info: String = ""
        ...
        init(
            ...
            defaultCabinet: Int,
            salary: RegistrySchemaV1.Salary,
            doctorSalary: RegistrySchemaV3.Salary,
            agentFee: Double = 0,
            agentFeePaymentDate: Date = .now,
            balance: Double = 0,
            info: String = "",
        ...
        public func charge(as role: KeyPath<MedicalService, Doctor?>, amount: Double) {
            switch role {
            case \.performer:
                balance += amount
            case \.agent:
